...
public class World {
	...
	private final char FIRST_TERRAIN = ' ', LAST_TERRAIN = '~'; // this covers the entire printable ASCII
	private Position[] spawnMarker = new Position[10]; // from digits 0 to 9

	private List<Robot> robots = new ArrayList<>();
	private Controller controller = new PhysicalController(this);
	...
	private int scepterPosX, scepterPosY;

	public static class Scepter {

		private static final Scepter INSTANCE = new Scepter();

		private static Scepter getInstance() {
			return INSTANCE;
		}
		...
		public String toString() {
			return "The one and only Scepter of Submission";
		}
	}
	...
	// Controller class listens to and stores all keyboard and mouse input events
	// and has getters to ask for the state
	public static class PhysicalController extends MouseAdapter implements KeyListener, Controller {
		/// Internal Attributes
		private World world;
		...
		public void keyPressed(KeyEvent e) {
			...
			// default keybindings for the UI buttons, you can remove or change these if you
			// want
	...
	public World(String mapData, Controller controller) {
		this(mapData, true);
		this.controller = controller;
	}

	public World(String mapData, String key, int keyPosX, int keyPosY, int scepterPosX, int scepterPosY) {
		this(mapData, true);
		this.key = key;
		this.keyPosX = keyPosX;
		this.keyPosY = keyPosY;
		this.scepterPosX = scepterPosX;
		this.scepterPosY = scepterPosY;
	}

	public World(String mapData, Controller controller, String key, int keyPosX, int keyPosY, int scepterPosX,
			int scepterPosY, boolean shouldRender) {
		this(mapData, shouldRender);
		this.key = key;
		this.keyPosX = keyPosX;
		this.keyPosY = keyPosY;
		this.scepterPosX = scepterPosX;
	...
	// you should probably leave shouldRender to true, otherwise you will not see
	// anything
	...
	public String getKey(int x, int y) {
		if (keyPosX == x && keyPosY == y) {
			return key;
		}
		return "Nice try!";
	}

	public Scepter getScepter(int x, int y, String key) {
		if (scepterPosX == x && scepterPosY == y && this.key.equals(key)) {
			return Scepter.getInstance();
		} else {
			return null;
		}
	}
	...
	public Position spawnRobotAt(Robot robot, char spawnMarker) {
		...
		if ('0' <= spawnMarker && spawnMarker <= '9') {
			...
			throw new IllegalArgumentException(
					"Invalid spawn marker '" + spawnMarker + "', it should be a single digit!");
	}
	...
	public boolean resolveCollision(Robot robot, Position pos) {
		...
		boolean xEdge = Math.ceil(pos.x - radius) == Math.floor(pos.x + radius), // this edge (vertical axis) intersects
				yEdge = Math.ceil(pos.y - radius) == Math.floor(pos.y + radius); // this edge (horizontal axis)
																					// intersects

		Position intersection = new Position(xEdge ? Math.ceil(pos.x - radius) : pos.x,
		...
		// "corner" case
			...
			// outer corner
				...
				intersection.x = blocked[0] || blocked[3] ? Math.min(pos.x, intersection.x)
						: Math.max(pos.x, intersection.x);
				intersection.y = blocked[0] || blocked[1] ? Math.min(pos.y, intersection.y)
						: Math.max(pos.y, intersection.y);
	...
	private class Renderer {
		...
		private BufferedImage[] ground = new BufferedImage[LAST_TERRAIN - FIRST_TERRAIN + 1]; // ground with potential
																								// text
		...
		void setup() {
			...
			JRadioButton[] r = { new JRadioButton("x0.25"), new JRadioButton("x0.5"), new JRadioButton("x1"),
					new JRadioButton("x2"), new JRadioButton("x4") };
			...
			// event handlers
			if (controller instanceof PhysicalController physicalController) {
				frame.addKeyListener(physicalController);
				frame.addMouseListener(physicalController);
				canvas.addMouseMotionListener(physicalController);
				canvas.addMouseListener(physicalController);
				frame.addMouseWheelListener(physicalController);
			}
		...
		private class Canvas extends JPanel {
			...
			protected void paintComponent(Graphics g) {
				...
				// g.drawOval(round(controller.mousePos.x * scale), round(controller.mousePos.y
				// * scale), 10, 10);
				...
				for (int y = 0; y < height; y++)
					for (int x = 0; x < width; x++) {
						...
						if (FIRST_TERRAIN <= c && c <= LAST_TERRAIN && c != '#')
							drawImage(g, ground[c - FIRST_TERRAIN], cameraX + (x + 0.5) * scale,
									cameraY + (y + 0.5) * scale, scale, 0);
					}
				...
				for (int z = 0; z < zResolution; z++) {
					if (pengu[z] != null)
						for (Robot robot : robots) {
							...
							drawImage(g, pengu[z], cameraX + pos.x * scale,
									cameraY + (pos.y + dz * z * size * 1.5) * scale, scale * size,
									robot.getDirection() + wiggle);
						}

					for (int y = 0; y < height; y++)
						for (int x = 0; x < width; x++)
							if (terrain[x][y] == '#' && ice[z] != null)
								drawImage(g, ice[z], cameraX + (x + 0.5) * scale,
										cameraY + (y + dz * z * 1.2 + 0.5) * scale, scale, 0);
				}
				...
				for (Robot robot : robots) {
					...
					if (speechTimer.get(robot) > 0) {
						...
						int[] yP = { y, y - arrowH, y - arrowH, y - arrowH - h, y - arrowH - h, y - arrowH,
								y - arrowH };
